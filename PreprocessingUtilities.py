...
def generate_mmsi_file(outputfolder, df):
    """
    ...
    df_mmsi.to_csv(outputfolder+"MMSIList.csv", index = False, mode='w', header=True, sep=',', index_label=None, encoding=None)
    return mmsi_set
...
def remove_unvalid_Lat_Long_duplicates(df_temp):
    ...
    df_temp.drop(df_temp[(df_temp['Longitude'] > 180) & (df_temp['Longitude']<-180)].index, inplace = True)
        # df_temp.drop(df[(df['Navigational status'] == 'Moored') & (df['SOG'] <= 0.1)].index, inplace = True)
        # df_temp.drop(df[(df['Navigational status'] == 'At anchor') & (df['SOG'] <= 0.1)].index, inplace = True)
        # df_temp.drop(df[(df['Navigational status'] == 'Unkown value') & (df['SOG'] <= 0.1)].index, inplace = True)
...
def change_timestamp_format(date, outputfolder, data_file):
    """
    ...
    df.to_csv(outputfolder+'aisdk-%s.csv'%date, sep=',',index=False,index_label=False, encoding='utf-8')
    return df
